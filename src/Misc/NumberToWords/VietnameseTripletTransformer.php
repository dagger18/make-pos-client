...
class VietnameseTripletTransformer implements TripletTransformer
{
    private VietnameseDictionary $dictionary;

    public function __construct(VietnameseDictionary $dictionary)
    {
    ...
    private function getSubHundred($tens, $units): string
    {
        ...
        if ($tens === 1) {
            ...
            if ($units > 0) {
                $words[] = $this->dictionary->{'getCorrespondingUnit' . ($tens > 0 ? 'Ten': '')}($units);
            }
        }
