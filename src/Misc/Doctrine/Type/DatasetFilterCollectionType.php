...
class DatasetFilterCollectionType extends Type implements ServiceTypeInterface
{
    ...
    {
        return 'DatasetFilterCollection';
    }
    ...
    public function convertToPHPValue($value, AbstractPlatform $platform)
    {
        ...
        $collection = new DatasetFilterCollection();
        forEach($array as $index=>$item) {
            // todo: abide for your sin :)))
            $item = array_combine($this->properties, $item);
            $entity = $this->serializer->deserialize(
                json_encode($item, JSON_THROW_ON_ERROR | JSON_PRESERVE_ZERO_FRACTION), DatasetFilter::class, 'json', 
